...
export default async function CandidateApplicationsPage() {
  ...
  const rows = (applications ?? []) as CandidateApplicationRow[];
              ...
              <ul className="space-y-3">
                {rows.map((application) => (
                  <li key={application.id} className="rounded-xl border border-slate-200 p-4">
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        ...
                        <p className="text-sm text-slate-600">
                          {application.jobs?.company_name ?? "Unknown company"} • {application.jobs?.location ?? "Unknown location"}
                        </p>
                    ...
                    {application.jobs ? (
                      <Link href={`/jobs/${application.jobs.id}`} className="mt-3 inline-block text-sm font-medium text-slate-900 underline-offset-4 hover:underline">
                        View job
                      </Link>
                    ) : null}
                  </li>
